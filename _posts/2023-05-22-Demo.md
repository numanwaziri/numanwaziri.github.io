...
{: .prompt-warning }
To **validate** the power-law hypothesis, One method is to **compare** the probability densities between the **theortical** power-law distribtion and the **emeperical** probabiilty distribution of the data with a <a href="https://en.wikipedia.org/wiki/Q%E2%80%93Q_plot" style="text-decoration: none;" target="_blank">Q-Q Plot</a>  having logarithmic axes also referred to as log-log plot.
1. The theorthical probability density  can be employed once we derive the $\alpha$ parameter for observed data - Fit the power-law distribution to data using  <a href="https://en.wikipedia.org/wiki/Maximum_likelihood_estimation" style="text-decoration: none;" target="_blank">maximum likelihood estimation</a> to retreive the scaling parameter $\alpha$ best representing the data.
> $$\hat{\alpha} = 1 + n \left(\sum_{i=1}^{n} \ln\left(\frac{x_i}{x_{\text{min}}}\right)\right)^{-1}$$

2. The emperical probability densities of name can be retrieved by aggregating the births with respect to each name and then calculating the relative frequency of births that fall within small intervals or "bins" (of names sorted by occurance/Birth) along the entirety of data.
